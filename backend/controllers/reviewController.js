...
const createReview = asyncHandler(async (req, res) => {
  ...
  if (await Review.findOne({ booking: bookingId })) { res.status(400); throw new Error("Already reviewed"); }
  const review = await Review.create({ booking: bookingId, customer: req.user._id, photographer: booking.photographer, rating, comment, aspects });
...
const getPhotographerReviews = asyncHandler(async (req, res) => {
  const reviews = await Review.find({ photographer: req.params.photographerId }).populate("customer", "name avatar").sort({ createdAt: -1 });
  res.json(reviews);
});
